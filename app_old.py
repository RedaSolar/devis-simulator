...
if mode == "Créer un Devis (1 ou 2 scénarios)":
    ...
    if use_ws_roi:
        ...
        if st.button("ðŸ§® Estimer les 12 mois (factures ROI)"):
    ...
    if st.button("âš™ï¸ Remplir automatiquement les lignes (panneaux, onduleurs, structures, smart meter, wifi)"):
    ...
    for i, line in enumerate(st.session_state.custom_lines_sans):
        ...
        with c7:
            remove = st.checkbox("âŒ", key=f"sans_rem_{i}")
    ...
    for i, line in enumerate(st.session_state.custom_lines_avec):
        ...
        with c7:
            remove = st.checkbox("âŒ", key=f"avec_rem_{i}")
    ...
    for i, note in enumerate(st.session_state.notes_sans):
        ...
        with col_del:
            delete = st.checkbox("âŒ", key=f"note_sans_del_{i}")
        if not delete:
    ...
    for i, note in enumerate(st.session_state.notes_avec):
        ...
        with col_del:
            delete = st.checkbox("âŒ", key=f"note_avec_del_{i}")
        if not delete:
    ...
    st.subheader("ðŸ“ˆ ROI (Retour sur investissement)")
